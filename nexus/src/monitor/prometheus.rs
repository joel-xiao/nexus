...
impl PrometheusMetrics {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let http_requests_total = Counter::with_opts(
            Opts::new("http_requests_total", "Total number of HTTP requests").namespace("nexus"),
        )?;
        ...
        let adapter_calls_total = Counter::with_opts(
            Opts::new("adapter_calls_total", "Total number of adapter calls").namespace("nexus"),
        )?;
        ...
        let adapter_errors_total = Counter::with_opts(
            Opts::new("adapter_errors_total", "Total number of adapter errors").namespace("nexus"),
        )?;
        registry.register(Box::new(adapter_errors_total.clone()))?;

        let task_queue_size = Gauge::with_opts(
            Opts::new("task_queue_size", "Current size of task queue").namespace("nexus"),
        )?;
        ...
        let tasks_failed_total = Counter::with_opts(
            Opts::new("tasks_failed_total", "Total number of failed tasks").namespace("nexus"),
        )?;
        registry.register(Box::new(tasks_failed_total.clone()))?;

        let cache_hits_total = Counter::with_opts(
            Opts::new("cache_hits_total", "Total number of cache hits").namespace("nexus"),
        )?;
        registry.register(Box::new(cache_hits_total.clone()))?;

        let cache_misses_total = Counter::with_opts(
            Opts::new("cache_misses_total", "Total number of cache misses").namespace("nexus"),
        )?;
