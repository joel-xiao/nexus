...
use crate::routes::config::flags::{CreateFlagRequest, FlagResponse, UpdateFlagRequest};
...
pub async fn check_flag(
    ...
    Json(FlagResponse { name, enabled })
}

pub async fn list_flags(Extension(state): Extension<Arc<AppState>>) -> Json<Vec<FeatureFlag>> {
...
pub async fn delete_flag(
    ...
    ok_response_with_message(
        &format!("Feature flag {} deleted", name),
        serde_json::json!({}),
    )
}
