...
impl PromptStore {
    ...
    pub fn render_with_template(&self, _name: &str, template: &str, variables: &Value) -> anyhow::Result<String> {
        let result = self.registry.render_template(template, &variables)?;
        Ok(result)
    }

    pub fn render_string(&mut self, template_str: &str, variables: &Value) -> anyhow::Result<String> {
        // 直接渲染给定的模板字符串，而不依赖内部 templates 映射
