...
use axum::{Router, Extension, middleware::Next};
...
pub fn create_app(
    ...
    if include_middleware {
        app = app.layer(
            axum::middleware::from_fn(|req: Request, next: Next| {
                async move {
                    error_middleware(req, next).await
                }
            })
        );
    }
...
pub fn create_test_app() -> Router {
    ...
    let prometheus_metrics = Arc::new(
        PrometheusMetrics::new().expect("Failed to create PrometheusMetrics")
    );
...
pub async fn metrics_handler(
    Extension(metrics): Extension<Arc<PrometheusMetrics>>,
) -> String {
